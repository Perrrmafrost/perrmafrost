...
6. **Render Sequence 1:** `compose_first_frames.py` for its shots (show `stills/first_frames.html`, approve), then `render_queue.py`, assemble it (`assemble_film.py ... --out seq01.mp4`), show the user, adjust settings if needed, then continue sequence by sequence (`git pull` for new shot files). Re-render bad takes (faces drifting, hands, text) with `--takes`, and record approvals in `renders/status.json`.
