...
- `film/shots/seq_NN_shots.jsonl` — per-shot, paste-ready video prompts (id, duration, prompt, negative, refs, flags). All 12 sequences are complete (1,781 shots); open questions are in `film/shots/OPEN_ITEMS.md` and none of them blocks rendering.
...
3. **Test clip.** The three workflows ship ready (`README.md` §2; validated against ComfyUI's schema, not yet run on a GPU). Download the models into the storage drive, then render one 5-second test at 848x480 from any 16:9 image (raise to 720p only if it fits and is fast enough). Fix whatever the first real run shows (file names, VRAM). Report seconds-per-clip and an honest estimate for the 1,781 shots x 2 takes (about 2 h 39 min of picture).
4. **Wire the batch renderer.** `config.json` already holds the node IDs; set `output_dir`/`stills_dir` to the storage drive and dry-run `render_queue.py --only <one id>` once that shot has a first frame.
5. **Reference stills first (consistency).** From the production bible, generate the core stills for the principals (Tut, Nour, Adaeze, Tomas, Tarek, Fathi, Rami, Hale, Layla, the forecast Akhenaten), the robot units (shabti, the Reis, jackal, fly, nurse, inch-worm) and the Seq 1-3 locations. Run `render_stills.py` (it reads `stills_manifest.jsonl`, whose priority-1 entries are exactly this set), open `stills/contact_sheet.html` with the user, and record their picks with `--approve`. Regenerate on request with `--ids ... --takes N`.
6. **Render Sequence 1:** `compose_first_frames.py` for its shots (show `stills/first_frames.html`, approve), then `render_queue.py`, assemble it (`assemble_film.py ... --out seq01.mp4`), show the user, adjust settings if needed, then continue sequence by sequence. Re-render bad takes (faces drifting, hands, text) with `--takes`, and record approvals in `renders/status.json`.
...
- **Time budget:** 1,781 shots × 2 takes × 5–8 min is about 300–475 GPU-hours (12–20 days non-stop), before upscaling and retakes. Confirm it with the step 3 timing.
