...
3. **Test clip.** The three workflows ship ready (`README.md` §2; validated against ComfyUI's schema, not yet run on a GPU). Download the models into the storage drive, then render one 5-second test at 848x480 from any 16:9 image (raise to 720p only if it fits and is fast enough). Fix whatever the first real run shows (file names, VRAM). Report seconds-per-clip and an honest estimate for ~1,200 shots x 2 takes.
