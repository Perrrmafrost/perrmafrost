...
3. **Test clip.** Build a working image-to-video workflow, render one 5-second test at ~832x480 (raise to 720p only if it fits and is fast enough). Report seconds-per-clip and an honest estimate for 1,781 shots x 2 takes (about 2 h 39 min of picture).
...
6. **Render Sequence 1**, assemble it (`assemble_film.py ... --out seq01.mp4`), show the user, adjust settings if needed, then continue sequence by sequence. Re-render bad takes (faces drifting, hands, text) with `--takes`, and record approvals in `renders/status.json`.
